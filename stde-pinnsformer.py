# TODO (1) : Implement the PINNsformer model (2) pseudo sequence generator (x,t) --> [(x,t), (x,t+delt),..., (x,t+T)] 
#
...
def make_time_sequence(
    x_array: jnp.ndarray,    # shape (batch_size, dim)
    t_start: float = 0.0,
    t_end: float = 1.0,
    num_steps: int = 5
) -> jnp.ndarray:
    """
    Returns an array of shape (batch_size, num_steps, dim+1),
    where each 'slice' along the time dimension is (x, t_i).
    """
    batch_size, d = x_array.shape
    times = jnp.linspace(t_start, t_end, num_steps)  # shape (num_steps,)

    # Replicate x across the time dimension
    x_tiled = jnp.repeat(x_array[:, None, :], repeats=num_steps, axis=1)
    # Expand 'times' so we can broadcast: shape (1, num_steps, 1)
    t_seq = jnp.repeat(times[None, :, None], repeats=batch_size, axis=0)
    # Concatenate x and t along the last axis
    return jnp.concatenate([x_tiled, t_seq], axis=-1)  # shape (batch_size, num_steps, d+1)
...
def sample_domain_fn_sequence(
    n_pts: int, rng: jax.Array, t_steps: int = 5
) -> jnp.ndarray:
    """Sample domain points and create a time sequence for each."""
    x, _, rng = sample_domain_fn(n_pts, rng)  # x shape (n_pts, dim)
    # Now build time sequences for each x
    x_seq = make_time_sequence(x, t_start=0.0, t_end=1.0, num_steps=t_steps)
    # x_seq shape: (n_pts, t_steps, dim+1)
    return x_seq, rng
...
class PINN(hk.Module):
    """PINN that uses a Transformer-based submodule instead of an MLP."""

    def __call__(self, x_seq: jnp.ndarray) -> jnp.ndarray:
        """
        x_seq has shape (batch_size, seq_len, d_in),
        where d_in = spatial_dim + 1 (last coordinate is time).
        """
        # The transformer returns shape (batch_size, seq_len, d_out).
        pred_seq = self.net(x_seq)

        # Suppose we want a final PDE solution for each (x, t). 
        # pred_seq will remain shape (batch_size, seq_len).
        # Then apply boundary or initial conditions as needed:
        return ZeroOnUnitBall_enforce_boundary_sequence(x_seq, pred_seq)

    def net(self, x_seq: jnp.ndarray) -> jnp.ndarray:
        # We can skip the reshape if x_seq is already (batch, seq, dim+1).
        # Pass directly to your Transformer:

        transformer = PINNsformer(
            d_out=1,         # output dimension is scalar
            d_hidden=32,
            d_model=32,
            N=1,
            num_heads=2
        )
        out = transformer(x_seq)  # shape (batch_size, seq_len, 1)

        # Squeeze the last dimension so shape => (batch, seq_len)
        return out[..., 0]


    def loss_fn(self, x):
        """
        Exactly as before: compute PDE residual, etc.
        (SineGordon_res_fn calls self(...) inside.)
        """
        domain_res = jax.vmap(lambda x_: SineGordon_res_fn(x_, self.__call__))(x)
        domain_loss = jnp.mean(domain_res**2)
        loss = domain_loss
    ...
    def err_norms_fn(self, x, y):
        """Compute L1 and L2 errors on test data, as before."""
        y_pred = self.__call__(x)
        err = y - y_pred
        l1 = jnp.abs(err).sum()
        l2 = (err**2).sum()
    ...
    def init_for_multitransform(self):
        """
        For your `hk.multi_transform` usage: return the apply functions
        with namedtuple. Exactly as your original code did.
        """
        return (
            self.__call__,
            namedtuple("PINN", ["u", "loss_fn", "err_norms_fn"])(
...
# WaveAct
# ---------------
class WaveAct(hk.Module):
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        """Wavelet-like activation with learnable w1, w2."""
        w1 = hk.get_parameter("w1", shape=[1], init=jnp.zeros)
        w2 = hk.get_parameter("w2", shape=[1], init=jnp.zeros)
...
class FeedForward(hk.Module):
    def __init__(self, d_model: int, d_ff: int = 256, name=None):
        super().__init__(name=name)
        self.d_model = d_model
        self.d_ff = d_ff

    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        x = hk.Linear(self.d_ff)(x)
        x = WaveAct()(x)
        x = hk.Linear(self.d_ff)(x)
        x = WaveAct()(x)
...
class MultiHeadAttention(hk.Module):
    """A simple multi-head attention for demonstration."""

    def __init__(self, num_heads: int, d_model: int, name=None):
        super().__init__(name=name)
        self.num_heads = num_heads
        self.d_model = d_model

    def __call__(self, x_q: jnp.ndarray, x_kv: jnp.ndarray) -> jnp.ndarray:
        """
        x_q, x_kv: shape (batch, seq_len, d_model)
        Returns:    shape (batch, seq_len, d_model)
        """
        head_dim = self.d_model // self.num_heads
        # Simple approach: linear transform for Q/K/V
        q = hk.Linear(self.d_model)(x_q)
        k = hk.Linear(self.d_model)(x_kv)
        v = hk.Linear(self.d_model)(x_kv)

        # Reshape for heads
        q = q.reshape(q.shape[0], q.shape[1], self.num_heads, head_dim)
        k = k.reshape(k.shape[0], k.shape[1], self.num_heads, head_dim)
        v = v.reshape(v.shape[0], v.shape[1], self.num_heads, head_dim)

        # Scale-dot product attention
        score = jnp.einsum("bqhd,bkhd->bhqk", q, k)
        scale = jnp.sqrt(head_dim).astype(score.dtype)
        attn_weights = jax.nn.softmax(score / scale, axis=-1)
        out = jnp.einsum("bhqk,bkhd->bqhd", attn_weights, v)

        # Merge heads back
        out = out.reshape(x_q.shape[0], x_q.shape[1], self.d_model)
        out = hk.Linear(self.d_model)(out)
        return out
...
class EncoderLayer(hk.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        x2 = WaveAct()(x)
        attn_output = MultiHeadAttention(self.num_heads, self.d_model)(x2, x2)
        x = x + attn_output

        x2 = WaveAct()(x)
        ff_output = FeedForward(self.d_model)(x2)
        x = x + ff_output
        return x
...
class DecoderLayer(hk.Module):
    ...
    def __call__(self, x, e_outputs):
        x2 = WaveAct()(x)
        cross_attn = MultiHeadAttention(self.num_heads, self.d_model)
...
class Encoder(hk.Module):
    def __init__(self, d_model, N, num_heads, name=None):
        super().__init__(name=name)
        self.d_model = d_model
        self.N = N
        self.num_heads = num_heads
...
# ---------------
# PINNsformer (Transformer) module
# ---------------
class PINNsformer(hk.Module):
    def __init__(self, d_out, d_hidden, d_model, N, num_heads, name=None):
        super().__init__(name=name)
        self.d_out = d_out
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        For a PDE in R^dim, you might treat each x_i as a single "token"
        or break it up differently. Adjust as needed.
        """
        # Project to d_model
        x = hk.Linear(self.d_model)(x)

        # Encode / Decode
        e_outputs = Encoder(self.d_model, self.N, self.num_heads)(x)
        d_output = Decoder(self.d_model, self.N, self.num_heads)(x, e_outputs)

        # Final feed-forward
        x = hk.Linear(self.d_hidden)(d_output)
        x = WaveAct()(x)
        x = hk.Linear(self.d_hidden)(x)
        x = WaveAct()(x)
        x = hk.Linear(self.d_out)(x)
        return x
